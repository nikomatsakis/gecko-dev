...
static inline CheckedInt32 alignTo(CheckedInt32 address, int32_t align) {
                            ...
                            HandleObject fields)
{
    ...
    for (unsigned int i = 0; i < ids.length(); i++) {
        ...
        CheckedInt32 offset = alignTo(sizeSoFar, fieldType->alignment());
        if (!offset.isValid()) {
            JS_ReportErrorNumber(cx, js_GetErrorMessage, nullptr,
                                 JSMSG_TYPEDOBJECT_TOO_BIG);
            return nullptr;
    ...
    CheckedInt32 totalSize = alignTo(sizeSoFar, alignment);
                          ...
                          int32_t length)
{
    ...
    switch (descr->kind()) {
      ...
      case TypeDescr::UnsizedArray:
      {
        ...
        Rooted<ArrayBufferObject*> buffer(cx);
        buffer = ArrayBufferObject::create(cx, totalSize.value(), false);
        if (!buffer)
            return nullptr;
