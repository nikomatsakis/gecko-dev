...
LIRGenerator::visitLoadTypedArrayElement(MLoadTypedArrayElement *ins)
{
    ...
    if (ins->arrayType() == type::TYPE_UINT32 && IsFloatingPointType(ins->type()))
        tempDef = temp();
...
LIRGenerator::visitStoreTypedArrayElement(MStoreTypedArrayElement *ins)
{
    ...
    if (ins->isFloatArray()) {
        ...
        JS_ASSERT_IF(optimizeFloat32 && ins->arrayType() == type::TYPE_FLOAT32,
                     ins->value()->type() == MIRType_Float32);
        JS_ASSERT_IF(!optimizeFloat32 || ins->arrayType() == type::TYPE_FLOAT64,
